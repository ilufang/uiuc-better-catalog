...
let parseSectionObj = function(sections, course) {
	...
	sections.forEach((section) => {
		...
		if (section.status.match(/section Pending/)) {
			secobj.status.code = 5;
			secobj.status.msg = "Pending";
		}
		...
		for (let i=0; i<meet_section.length; ++i) {
			let day = meet_day[i].innerText.trim().toUpperCase();
			if (day == "N.A") {
				day = [];
			} else {
				day = day.split('');
...
Course.prototype.stringify = function() {
	return JSON.stringify(this);
}

Course.prototype.getCredit = function() {
	return Course.getCredit(this.credit);
}

Course.prototype.getColor = function() {
	var colors = ['red','pink','purple','indigo','blue','cyan','teal','green','orange','brown'];
	var hue = 5381;
	var seed = this.dept; // +"/"+catalog.courses[this.dept].name;
	for (var i=0; i<seed.length; ++i) {
		hue = (hue*33) ^ seed.charCodeAt(i);
	}
	hue %= colors.length;
	return colors[hue];
}
...
Course.templateFromCode = function(code, title) {
	var courseobj = {};

	courseobj.code = code;
	var matches = /^([A-Z]{2,}) (\d{3,})/.exec(code);
	if (!matches)
		throw 'Cannot parse course code';
	courseobj.dept = matches[1].toUpperCase();
	courseobj.number = matches[2];
	...
	courseobj.description = "Loading course info...";
	courseobj.additional = [];
