...
var Catalog = function(depts, courses) {
	...
	this.getCourse = function(code) {
		...
		if (!code || !this.courses[code.dept] || !this.courses[code.dept][course.number]) {
			return null;
		}
		return this.courses[code.dept][course.number];
	}
	...
	return new Promise((resolve, reject) => {
		io.getItem('schedule/'+term).then((val) => {
			...
			table.querySelectorAll("tr").forEach((dept) => {
				...
				depts[dept_code] = {name: dept_name};
...
Catalog.fromTerm = function(term, baseURL) {
	...
	return new Promise((resolve, reject) => {
		getTerm(term, io).then((val) => {
			try {
				var catalog = new Catalog(val.depts, val.courses);
...
Catalog.fromStorage = function(storage) {
	...
	let catalog = new Catalog(depts, courses);
